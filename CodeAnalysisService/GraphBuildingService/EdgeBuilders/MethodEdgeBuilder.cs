...
namespace CodeAnalysisService.GraphBuildingService.EdgeBuilder
{
    ...
    public class MethodEdgeBuilder : IEdgeBuilder
    {
        ...
        public IEnumerable<EdgeNode> BuildEdges(INode node, NodeRegistry registry, SemanticModel model)
        {
            ...
            var descendants = methodNode.MethodSyntax.DescendantNodes();
            var collectionVariables = descendants.OfType<ForEachStatementSyntax>().ToDictionary(f => f.Identifier.Text, f =>
                    {
                        var collectionType = model.GetTypeInfo(f.Expression).Type;
                        if (collectionType == null)
                            return null;

                        return TypeHelper.GetElementType(collectionType) ?? collectionType;
                    },
